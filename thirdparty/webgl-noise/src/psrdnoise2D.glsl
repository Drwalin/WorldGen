...
//    where float(0.0) is no rotation and float(1.0) is one full turn)
...
// (The constant float(0.0243902439) is 1/41)
vec2 rgrad2(vec2 p, float rot) {
#if 0
// Map from a line to a diamond such that a shift maps to a rotation.
  float u = permute(permute(p.x) + p.y) * float(0.0243902439) + rot; // Rotate by shift
  u = float(4.0) * fract(u) - float(2.0);
  // (This vector could be normalized, exactly or approximately.)
  return vec2(abs(u)-float(1.0), abs(abs(u+float(1.0))-float(2.0))-float(1.0));
#else
// For more isotropic gradients, sin/cos can be used instead.
  float u = permute(permute(p.x) + p.y) * float(0.0243902439) + rot; // Rotate by shift
  u = fract(u) * float(6.28318530718); // 2*pi
...
vec3 psrdnoise(vec2 pos, vec2 per, float rot) {
  ...
  vec2 uv = vec2(pos.x + pos.y*float(0.5), pos.y);
  
  vec2 i0 = floor(uv);
  vec2 f0 = fract(uv);
  // Traversal order
  vec2 i1 = (f0.x > f0.y) ? vec2(float(1.0), float(0.0)) : vec2(float(0.0), float(1.0));

  // Unskewed grid points in (x,y) space
  vec2 p0 = vec2(i0.x - i0.y * float(0.5), i0.y);
  vec2 p1 = vec2(p0.x + i1.x - i1.y * float(0.5), p0.y + i1.y);
  vec2 p2 = vec2(p0.x + float(0.5), p0.y + float(1.0));

  // Integer grid point indices in (u,v) space
  i1 = i0 + i1;
  vec2 i2 = i0 + vec2(float(1.0), float(1.0));
  ...
  // float(0.8) is the square of 2/sqrt(5), the distance from
  // a grid point to the nearest simplex boundary
  vec3 t = float(0.8) - vec3(dot(d0, d0), dot(d1, d1), dot(d2, d2));

  // Partial derivatives for analytical gradient computation
  vec3 dtdx = -float(2.0) * vec3(d0.x, d1.x, d2.x);
  vec3 dtdy = -float(2.0) * vec3(d0.y, d1.y, d2.y);

  // Set influence of each surflet to zero outside radius sqrt(float(0.8))
  if (t.x < float(0.0)) {
    dtdx.x = float(0.0);
    dtdy.x = float(0.0);
	t.x = float(0.0);
  ...
  vec2 dt0 = vec2(dtdx.x, dtdy.x) * float(4.0) * t3.x;
  vec2 dn0 = t4.x * g0 + dt0 * w.x;
  vec2 dt1 = vec2(dtdx.y, dtdy.y) * float(4.0) * t3.y;
  vec2 dn1 = t4.y * g1 + dt1 * w.y;
  vec2 dt2 = vec2(dtdx.z, dtdy.z) * float(4.0) * t3.z;
  vec2 dn2 = t4.z * g2 + dt2 * w.z;

  return float(11.0)*vec3(n, dn0 + dn1 + dn2);
}
...
//
vec3 psdnoise(vec2 pos, vec2 per) {
  return psrdnoise(pos, per, float(0.0));
}
...
//
float psnoise(vec2 pos, vec2 per) {
  return psrnoise(pos, per, float(0.0));
}
