...
vec2 fade(vec2 t) { return t * t * t * (t * (t * float(6.0) - float(15.0)) + float(10.0)); }

// Classic Perlin noise
float perlin_cnoise(vec2 P)
{
	vec4 Pi = floor(vec4(P.x, P.y, P.x, P.y)) + vec4(float(0.0), float(0.0), float(1.0), float(1.0));
	vec4 Pf = fract(vec4(P.x, P.y, P.x, P.y)) - vec4(float(0.0), float(0.0), float(1.0), float(1.0));
	...
	vec4 gx = fract(i * (float(1.0) / float(41.0))) * float(2.0) - float(1.0);
...
float perlin_pnoise(vec2 P, vec2 rep)
{
	vec4 Pi = floor(vec4(P.x, P.y, P.x, P.y)) + vec4(float(0.0), float(0.0), float(1.0), float(1.0));
	vec4 Pf = fract(vec4(P.x, P.y, P.x, P.y)) - vec4(float(0.0), float(0.0), float(1.0), float(1.0));
