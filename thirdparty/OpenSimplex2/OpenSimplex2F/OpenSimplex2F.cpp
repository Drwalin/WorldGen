...
{
	static Grad2 *arr = NULL;
	if (arr == NULL) {
		arr = (Grad2 *)malloc(sizeof(Grad2) * 24);
		int i = 0;
...
{
	static Grad3 *arr = NULL;
	if (arr == NULL) {
		arr = (Grad3 *)malloc(sizeof(Grad3) * 48);
		int i = 0;
...
Grad4 *_newGrad4ConstArray(){
	static Grad4 *arr = NULL;
	if(arr == NULL) {
		arr = (Grad4 *)malloc(sizeof(Grad4) * 160);
		int i = 0;
...
void FreeOSG(OpenSimplexGradients *osg)
{
    free(osg->perm);
    free(osg->permGrad2);
    free(osg->permGrad3);
